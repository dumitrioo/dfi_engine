    ...
    public:
        ...
        static std::string type_to_str(type t) {
            static std::array<std::string_view, 25> const names{
                ...
                "f32", "f64", "f128", "vec4", "mat4", "pointer", "class", "func",
        ...
        static bool is_type(std::string const &idr) {
            static std::set<std::string> const tset{
                ...
                "i16", "u32", "i32", "u64", "i64", "f32", "f64", "f128", "vec4", "mat4", "func",
...
#undef VALBOX_ASSIGN_PTR_OPERATOR_BODY
        ...
        valbox &operator+=(valbox const &rarg) {
            ...
            switch(lt) {
                ...
                case type::LONG_DOUBLE: lr.as_long_double() += rr.cast_to_long_double(); return *this;
                case type::VEC4: {
                        if(rr.is_numeric()) {
        ...
        valbox &operator*=(valbox const &rarg) {
            ...
            auto rt{rr.val_or_pointed_type()};
            switch(lt) {
                case type::BOOL: lr.as_bool() = lr.as_bool() && rr.cast_to_bool(); return *this;
                case type::CHAR: lr.as_char() *= rr.cast_to_char(); return *this;
                case type::S8: lr.as_s8() *= rr.cast_to_s8(); return *this;
                case type::U8: lr.as_u8() *= rr.cast_to_u8(); return *this;
                case type::S16: lr.as_s16() *= rr.cast_to_s16(); return *this;
        ...
        friend valbox operator%(valbox const &lr, valbox const &rr) {
            ...
            switch(lt) {
                ...
                case type::LONG_DOUBLE:
                    switch(rt) {
                        case type::BOOL:        { auto dvzr{rr.as_bool()};        return std::fmod(lr.as_long_double(), dvzr); }
                        case type::CHAR:        { auto dvzr{rr.as_char()};        return std::fmod(lr.as_long_double(), dvzr); }
                        case type::S8:          { auto dvzr{rr.as_s8()};          return std::fmod(lr.as_long_double(), dvzr); }
                        case type::U8:          { auto dvzr{rr.as_u8()};          return std::fmod(lr.as_long_double(), dvzr); }
                        case type::S16:         { auto dvzr{rr.as_s16()};         return std::fmod(lr.as_long_double(), dvzr); }
                        case type::U16:         { auto dvzr{rr.as_u16()};         return std::fmod(lr.as_long_double(), dvzr); }
                        case type::WCHAR:       { auto dvzr{rr.as_wchar()};       return std::fmod(lr.as_long_double(), dvzr); }
                        case type::S32:         { auto dvzr{rr.as_s32()};         return std::fmod(lr.as_long_double(), dvzr); }
                        case type::U32:         { auto dvzr{rr.as_u32()};         return std::fmod(lr.as_long_double(), dvzr); }
                        case type::S64:         { auto dvzr{rr.as_s64()};         return std::fmod(lr.as_long_double(), dvzr); }
                        case type::U64:         { auto dvzr{rr.as_u64()};         return std::fmod(lr.as_long_double(), dvzr); }
                        case type::FLOAT:       { auto dvzr{rr.as_float()};       return std::fmod(lr.as_long_double(), dvzr); }
                        case type::DOUBLE:      { auto dvzr{rr.as_double()};      return std::fmod(lr.as_long_double(), dvzr); }
                        case type::LONG_DOUBLE: { auto dvzr{rr.as_long_double()}; return std::fmod(lr.as_long_double(), dvzr); }
                        case type::VEC4: break;
                        case type::MAT4: break;
                        case type::POINTER: break;
                        case type::CLASS: break;
        ...
        valbox &assign_no_deref(valbox const &that) {
            if(box_.get() != nullptr && box_.get() == that.box_.get()) {
                return *this;
            }
            if(that.is_undefined()) {
                if(box_) {
                    std::unique_lock l{*box_};
                    box_->type_ = type::UNDEFINED;
                    box_->pointed_type_ = type::UNDEFINED;
                    box_->class_.clear();
                    box_->func_name_.clear();
                    box_->user_func_ = false;
                }
            } else {
                if(!box_) {
                    box_ = std::make_shared<box_data>(
                        that.box_->value_,
                        that.box_->type_,
                        that.box_->pointed_type_,
                        that.box_->class_,
                        that.box_->func_name_,
                        that.box_->user_func_
                    );
                } else {
                    std::unique_lock l{*box_};
                    box_->value_ = that.box_->value_;
                    box_->type_ = that.box_->type_;
                    box_->pointed_type_ = that.box_->pointed_type_;
                    box_->class_ = that.box_->class_;
                    box_->func_name_ = that.box_->func_name_;
                    box_->user_func_ = that.box_->user_func_;
                }
            }
            return *this;
        }
        ...
        std::string dump() const {
            ...
            switch(val_or_pointed_type()) {
                ...
                case type::MAT4: {
                    ...
                    break;
                case type::OBJECT: {
                        auto const &o{as_object()};
                        os << "{";
                        std::string sep{};
                        for(auto &&p: o) {
                            os << sep << p.first << ":" << p.second.dump();
                            sep = ",";
                        }
                        os << "}";
                    }
        ...
        std::wstring wdump() const {
            ...
            switch(val_or_pointed_type()) {
                ...
                case valbox::type::OBJECT: {
                        ...
                        std::wstring sep{};
                        for(auto &&p: o) {
                            os << sep << str_util::from_utf8(p.first) << ":" << p.second.wdump();
                            sep = L",";
                        }
                        os << L"}";
                    }
    ...
    static std::wostream &operator<<(std::wostream &os, valbox const &v) {
        switch(v.val_or_pointed_type()) {
            ...
            case valbox::type::OBJECT: {
                    ...
                    std::wstring sep{};
                    for(auto &&p: o) {
                        os << sep << str_util::from_utf8(p.first) << ":" << p.second;
                        sep = L",";
                    }
                    os << L"}";
                }
