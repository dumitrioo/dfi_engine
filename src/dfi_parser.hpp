    ...
    private:
        ...
        json get_func_def_statement() {
            ...
            if(
                ...
                increment_pos(2);
                res["content"]["arg_names"] = get_func_def_args_list()["content"];
                res["content"]["function_body"] = get_statement();
            }
            return res;
        }

        json get_func_def_args_list() {
            json res{};
        ...
        json get_cell_instantiation_statement() {
            json res{};
            if(
                get_token(0).is_id() && get_token(0).lexem() != L"function" &&
                get_token(1).is_id() &&
                get_token(2).type_is(token::type::LPAREN)
            ) {
                res["loc"]["line"] = get_token(0).line();
        ...
        json get_range_for_statement() {
            json res{};
            if(
                get_token(0).is_id() && get_token(0).lexem() == L"for" &&
                get_token(1).tktype() == token::type::LPAREN &&
                get_token(2).is_id() && !is_keyword(get_token(2).lexem()) &&
                get_token(3).tktype() == token::type::COLON
            ) {
                res["loc"]["line"] = get_token(0).line();
                res["loc"]["col"] = get_token(0).col();
                loops_nesting_.push_front("range_for");
                dfi::shut_on_destroy pop_nest{[&]() {
                    loops_nesting_.pop_front();
                }};
                increment_pos(2);
                json slider{};
                slider["loc"]["line"] = get_token(0).line();
                slider["loc"]["col"] = get_token(0).col();
                slider["type"] = "expression";
                slider["subtype"] = "identifier";
                slider["content"] = get_token(0).lexem();
                increment_pos(2);
                check_eof();
                json range_expr{get_expr()};
                if(range_expr.is_object()) {
                    check_eof();
                    if(get_token(0).type_is(token::type::RPAREN)) {
                        increment_pos();
                        auto stmt{get_statement()};
                        if(stmt.is_object()) {
                            res["type"] = "statement";
                            res["subtype"] = "range_for";
                            res["content"]["slider"] = slider;
                            res["content"]["range"] = range_expr;
                            res["content"]["statement"] = stmt;
                        } else {
                            throw compilation_error{
                                get_token(0).line(),
                                get_token(0).col(),
                                "invalid \"range for\": loop body expected"
                            };
                        }
                    } else {
                        throw compilation_error{
                    ...
                    throw compilation_error{
                        get_token(0).line(),
                        get_token(0).col(),
                        "invalid \"range for\": expression expected"
                    };
                }
            }
            return res;
            ...
            if(
                ...
                if(must_colon_tk.type_is(token::type::COLON)) {
                    ...
                    throw compilation_error{get_token(0).line(), get_token(0).col(),
                        std::string{"\":\" expected, got \""} + dfi::str_util::to_utf8(get_token(0).lexem()) + "\""
                    };
                }
            }
            return res;
        ...
        json get_primary() {
            ...
            if(tk.type_is(token::type::FP_LITERAL) || tk.type_is(token::type::FP_EXP_LITERAL)) {
                ...
                res["subtype"] = "literal";
                res["literal"] = "flt";
                res["content"] = std::stold(dfi::str_util::to_utf8(tk.lexem()));
                return res;
                ...
                bool bam{false};
                try {
                    res["content"] = dfi::str_util::atoi(dfi::str_util::to_utf8(tk.lexem()), 10, true);
                ...
                bool bam{false};
                try {
                    res["content"] = dfi::str_util::atoui(dfi::str_util::to_utf8(tk.lexem()), 16, true);
                } catch (std::exception const &e) {
                    msg = e.what();
                    bam = true;
                }
                ...
                bool bam{false};
                try {
                    res["content"] = dfi::str_util::atoui(dfi::str_util::to_utf8(tk.lexem()), 8, true);
                } catch (std::exception const &e) {
                    msg = e.what();
                    bam = true;
                }
                ...
                bool bam{false};
                try {
                    res["content"] = dfi::str_util::atoui(dfi::str_util::to_utf8(tk.lexem()), 2, true);
                } catch (std::exception const &e) {
                    msg = e.what();
                    bam = true;
                }
                ...
                res["subtype"] = "literal";
                res["literal"] = "str";
                res["content"] = dfi::str_util::to_utf8(tk.lexem());
                return res;
