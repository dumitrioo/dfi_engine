    ...
    private:
        void process_input_char(std::int64_t c, bool encountered = true) {
            ...
            if(state_.empty()) {
                ...
                if(dfi::str_util::fltr<std::wstring>::isdigit(c)) { buff_ += c; state_ = "int";
                } else if(dfi::str_util::fltr<std::wstring>::isspace(c)) { buff_ += c; state_ = "space";
        ...
        static bool is_ident_start(std::int64_t c) {
            return dfi::str_util::fltr<std::wstring>::isalpha(c) || c == '_' || c == '$';
        }

        static bool is_ident_tail(std::int64_t c) {
            return is_ident_start(c) || dfi::str_util::fltr<std::wstring>::isdigit(c);
        }

        void num(std::int64_t c) {
            if(dfi::str_util::fltr<std::wstring>::isdigit(c)) {
                buff_ += c;
            } else if(c == '\'') {
        ...
        void flt(std::int64_t c) {
            ...
            } else if(dfi::str_util::fltr<std::wstring>::tolower(c) == 'e') {
                if(float_phase_ != ".") {
                ...
                try {
                    switch(str_num_escape_mode_) {
                        case str_num_escaping_t::oct: {
                                ...
                                std::int64_t chr{
                                    (std::int64_t)dfi::str_util::atoi(
                                        dfi::str_util::to_utf8(esc_buff_),
                                        8
                                    )
                                };
                                if(esc_buff_.size() == 3) {
        ...
        void space(std::int64_t c) {
            if(!dfi::str_util::fltr<std::wstring>::isspace(c)) {
