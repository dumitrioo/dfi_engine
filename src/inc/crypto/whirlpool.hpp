    ...
    namespace crypt {
        ...
        private:
            ...
            void rho(std::uint64_t &b, std::uint64_t *a, std::uint64_t n, std::uint64_t c) {
                b = t_[(a[n] >> 56) & 0xFFULL];
                b ^= dfi::bit_util::ror<std::uint64_t>(t_[(a[(n + 7) % 8] >> 48) & 0xFFULL], 8).val;
                b ^= dfi::bit_util::ror<std::uint64_t>(t_[(a[(n + 6) % 8] >> 40) & 0xFFULL], 16).val;
                b ^= dfi::bit_util::ror<std::uint64_t>(t_[(a[(n + 5) % 8] >> 32) & 0xFFULL], 24).val;
                b ^= dfi::bit_util::ror<std::uint64_t>(t_[(a[(n + 4) % 8] >> 24) & 0xFFULL], 32).val;
                b ^= dfi::bit_util::ror<std::uint64_t>(t_[(a[(n + 3) % 8] >> 16) & 0xFFULL], 40).val;
                b ^= dfi::bit_util::ror<std::uint64_t>(t_[(a[(n + 2) % 8] >> 8) & 0xFFULL], 48).val;
                b ^= dfi::bit_util::ror<std::uint64_t>(t_[a[(n + 1) % 8] & 0xFFULL], 56).val;
                b ^= c;
            }
