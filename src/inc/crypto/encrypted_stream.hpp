...
namespace dfi::crypt {
    ...
    public:
        ...
        void set_key(void const *k, std::size_t k_len) {
            if(k_len == 64) {
                ...
                key_ = dfi::crypt::sha3_512_compute(k, k_len);
            }
            nonce_ = dfi::crypt::sha3_512_compute(key_);;
        }
    ...
    private:
        ...
        ssize_type content_size() {
            ...
            std::uint64_t sz{dfi::bit_util::swap_on_be<std::uint64_t>{*(std::uint64_t *)decrypted_read_data.data()}.val};
            return sz;
        }
        ...
        bool set_content_size(ssize_type s) {
            if(s >= 0 && file_ptr_ && file_ptr_->is_open()) {
                ...
                std::uint64_t sz{dfi::bit_util::swap_on_be<std::uint64_t>{(std::uint64_t)s}.val};
    ...
    private:
        std::array<std::uint8_t, CIPHER_T::iv_length()> iv(std::int64_t block_no) const {
            ...
            std::uint64_t block_no_le{dfi::bit_util::swap_on_be<std::uint64_t>{(std::uint64_t)block_no}.val};
