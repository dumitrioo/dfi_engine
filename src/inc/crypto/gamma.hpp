...
namespace dfi::crypt {
    ...
    public:
        gamma() {
            std::array<std::uint8_t, dfi::crypt::threefish<512>::key_length()> key{};
        ...
        gamma(dfi::bytevec const &key, dfi::bytevec const &iv) {
            init(key, iv);
        }
        ...
        static std::size_t constexpr key_size() {
            return dfi::crypt::threefish<512>::key_length();
        }

        static std::size_t constexpr block_size() {
            return dfi::crypt::threefish<512>::block_size();
        }

        static std::size_t constexpr iv_length() {
            return dfi::crypt::threefish<512>::iv_length() - 8;
        }
        ...
        void init(dfi::bytevec const &key, dfi::bytevec const &iv = {}) {
    ...
    private:
        void merge_iv() {
            std::uint64_t curr_block_no{dfi::bit_util::swap_on_be<std::uint64_t>{curr_block_no_}.val};
            memcpy(&iv_[0], &curr_block_no, 8);
        }

    private:
        std::array<std::uint8_t, dfi::crypt::threefish<512>::key_length()> key_{};
        std::array<std::uint8_t, dfi::crypt::threefish<512>::iv_length()> iv_{};
        std::uint64_t curr_block_no_{0xffffffffffffffffULL};
        std::array<std::uint8_t, dfi::crypt::threefish<512>::block_size()> curr_block_{};
        dfi::crypt::threefish<512> cipher_{};
