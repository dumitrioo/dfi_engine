...
#pragma pack(push, 1)
    namespace detail {
        ...
        static std::uint64_t n_size(std::uint8_t first) {
            ...
            first |= first >> 2;
            first |= first >> 4;
            return 9 - dfi::bit_util::num_of_set_bits(first);
        }
        ...
        class size_surrounded_buffer final {
            friend class dfi::serializer;
            friend class dfi::serial_reader;
        ...
        private:
            ...
            void set_sizes(size_type s) {
                ...
                dfi::bit_util::inplace_swap(reinterpret_cast<std::uint8_t *>(data()) + s, size_size());
            }
                    ...
                    if((std::int64_t)(ss * 2) <= whole_size_limit) {
                        ...
                        if((std::int64_t)(ss * 2 + s) <= whole_size_limit) {
                            ...
                            if(bss == ss) {
                                ...
                                std::uint64_t bs{};
                                if(ss > 1) { dfi::bit_util::inplace_swap(buff, ss); }
            ...
            size_surrounded_buffer *prev() const {
                ...
                dfi::bit_util::inplace_swap(buff, prev_size_size);
    ...
    public:
        ...
        serializer(dfi::bytevec const &data):
            serialized_data_{data} {
        }

        serializer(dfi::bytevec &&data):
            serialized_data_{std::move(data)} {
        }

        serializer &operator=(dfi::bytevec const &data) {
            serialized_data_ = data;
            return *this;
        }

        serializer &operator=(dfi::bytevec &&data) {
            serialized_data_ = std::move(data);
            return *this;
        }

        dfi::bytevec const &data_vec() const & {
            return serialized_data_;
        }

        dfi::bytevec take_vec() {
        ...
        void push_back(dfi::bytevec const &vec) {
        ...
        serializer &operator<<(dfi::bytevec const &vec) {
            push_back(vec);
            return *this;
        }
    ...
    public:
        ...
        serial_reader(dfi::bytevec const &data):
