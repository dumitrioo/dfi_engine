                ...
                switch(rule_parse_state_) {
                    case parse_states::initial:
                        if(escape) {
                            ...
                            if(ch == RULES_SEPARATOR) {
                                ...
                                state_name = dfi::str_util::trim(state_name);
                                if(state_name.empty()) {
                    ...
                    case parse_states::rule:
                        if(buf_scope + ch == RULE_TO_TARGET_TRANSITION) {
                            ...
                            if(symbol_or_class.size() > 2) {
                                ...
                                symbol_or_class = dfi::str_util::trim(symbol_or_class);
                    ...
                    case parse_states::target:
                        if(escape) {
                            ...
                            if(ch == RULES_SEPARATOR) {
                                next_state = dfi::str_util::trim(next_state);
                                if(next_state.empty()) {
                                    throw std::runtime_error{"error in rules"};
                                }
                                add_rule(
                                    dfi::str_util::trim(state_name),
                                    dfi::str_util::trim(symbol_or_class),
                                    dfi::str_util::trim(next_state)
                                );
    ...
    private:
        ...
        bool match(CHAR_T sym, bool eof, const STR_T &sym_or_class) {
            ...
            if(sym_or_class[0] == '\'') {
                    ...
                    switch (ls) {
                        ...
                        case local_states::class_body:
                            if(cc == '!') {
                                ...
                                ls = local_states::end;
                                continue;
                            } else if(dfi::str_util::string_starts_from(sym_or_class.substr(i), CATEGORY_SPACE)) {
                                if(local_not_cond) {
                                    not_spaces = true;
                                } else {
                                    spaces = true;
                                ...
                                continue;
                            } else if(dfi::str_util::string_starts_from(sym_or_class.substr(i), CATEGORY_ALPHA)) {
                                if(local_not_cond) {
                                    not_alphas = true;
                                } else {
                                    alphas = true;
                                ...
                                continue;
                            } else if(dfi::str_util::string_starts_from(sym_or_class.substr(i), CATEGORY_ALNUM)) {
                                ...
                                continue;
                            } else if(dfi::str_util::string_starts_from(sym_or_class.substr(i), CATEGORY_CNTRL)) {
                                if(local_not_cond) not_cntrl = true; else cntrl = true;
                                i += CATEGORY_CNTRL.size() - 1;
                                continue;
                            } else if(dfi::str_util::string_starts_from(sym_or_class.substr(i), CATEGORY_DIGIT)) {
                                if(local_not_cond) not_digits = true; else digits = true;
                                i += CATEGORY_DIGIT.size() - 1;
                                continue;
                            } else if(dfi::str_util::string_starts_from(sym_or_class.substr(i), CATEGORY_PUNCT)) {
                                if(local_not_cond) not_puncts = true; else puncts = true;
                                i += CATEGORY_PUNCT.size() - 1;
                                continue;
                            } else if(dfi::str_util::string_starts_from(sym_or_class.substr(i), CATEGORY_HEX)) {
                                if(local_not_cond) not_hex = true; else hex = true;
                                i += CATEGORY_HEX.size() - 1;
                                continue;
                            } else if(dfi::str_util::string_starts_from(sym_or_class.substr(i), CATEGORY_OCT)) {
                ...
                if(not_spaces) { if(dfi::str_util::fltr<STR_T>::isspace(sym)) { neg_found = true; } }
                if(not_alphas) { if(dfi::str_util::fltr<STR_T>::isalpha(sym)) { neg_found = true; } }
                if(not_digits) { if(dfi::str_util::fltr<STR_T>::isdigit(sym)) { neg_found = true; } }
                if(not_puncts) { if(dfi::str_util::fltr<STR_T>::ispunct(sym)) { neg_found = true; } }
                if(not_cntrl) { if(dfi::str_util::fltr<STR_T>::iscntrl(sym)) { neg_found = true; } }
                if(not_hex) { if(dfi::str_util::fltr<STR_T>::ishexdigit(sym)) { neg_found = true; } }
                if(not_oct) { if(dfi::str_util::fltr<STR_T>::isoctdigit(sym)) { neg_found = true; } }
                ...
                if(spaces) { pos_given = true; if(dfi::str_util::fltr<STR_T>::isspace(sym)) { pos_found = true; } }
                if(alphas) { pos_given = true; if(dfi::str_util::fltr<STR_T>::isalpha(sym)) { pos_found = true; } }
                if(digits) { pos_given = true; if(dfi::str_util::fltr<STR_T>::isdigit(sym)) { pos_found = true; } }
                if(puncts) { pos_given = true; if(dfi::str_util::fltr<STR_T>::ispunct(sym)) { pos_found = true; } }
                if(cntrl) { pos_given = true; if(dfi::str_util::fltr<STR_T>::iscntrl(sym)) { pos_found = true; } }
                if(hex) { pos_given = true; if(dfi::str_util::fltr<STR_T>::ishexdigit(sym)) { pos_found = true; } }
                if(oct) { pos_given = true; if(dfi::str_util::fltr<STR_T>::isoctdigit(sym)) { pos_found = true; } }
        ...
        void accept(CHAR_T sym, bool eof, bool repeat = false) {
            ...
            if(state_map_it == rules_.end()) {
                std::stringstream ss{};
                ss << "at line " << current_l_ + 1 << ", col " << current_c_ + 1 << ": no rules for transition from state \"" << dfi::str_util::fltr<STR_T>::utf8(state_current_) << "\"";
        ...
        STR_T RULE_TO_TARGET_TRANSITION{dfi::str_util::fltr<STR_T>::cast(">>")};
        STR_T STATE_INITIAL{dfi::str_util::fltr<STR_T>::cast("#rd")};
        STR_T STATE_ERROR{dfi::str_util::fltr<STR_T>::cast("#er")};
        STR_T CLASS_DEFAULT{dfi::str_util::fltr<STR_T>::cast("$df")};
        STR_T CLASS_EOF{dfi::str_util::fltr<STR_T>::cast("$ef")};
        STR_T CATEGORY_SPACE{dfi::str_util::fltr<STR_T>::cast(":space:")};
        STR_T CATEGORY_ALPHA{dfi::str_util::fltr<STR_T>::cast(":alpha:")};
        STR_T CATEGORY_ALNUM{dfi::str_util::fltr<STR_T>::cast(":alnum:")};
        STR_T CATEGORY_CNTRL{dfi::str_util::fltr<STR_T>::cast(":cntrl:")};
        STR_T CATEGORY_DIGIT{dfi::str_util::fltr<STR_T>::cast(":digit:")};
        STR_T CATEGORY_PUNCT{dfi::str_util::fltr<STR_T>::cast(":punct:")};
        STR_T CATEGORY_HEX{dfi::str_util::fltr<STR_T>::cast(":hex:")};
        STR_T CATEGORY_OCT{dfi::str_util::fltr<STR_T>::cast(":oct:")};
        CHAR_T ESCAPE_CHAR{'\\'};
