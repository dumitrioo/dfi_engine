...
namespace dfi::file_util {
    ...
    static std::string extract_file_ext(const std::string &file_name) {
        ...
        std::vector<std::string> tokens = dfi::str_util::str_tok<std::string>(file_name, ".");
        if(tokens.size() < 2) {
            return "";
        }
...
#if (__cplusplus < 201700L)
        std::vector<std::string> tokens = dfi::str_util::str_tok<std::string>(path, native_path_separator<std::string>::val(), true);
    ...
    static std::string suffix_file_name_number(std::string const &fn, int num) {
        std::string tfn{fn};
        std::string sn{dfi::str_util::itoa<std::string>(static_cast<std::int64_t>(num))};
    ...
    public:
        ...
        dfi::timespec_wrapper modify_time() const { return modify_time_; }
    ...
    public:
        ...
        dfi::timespec_wrapper modify_time_{};
    };
    ...
    void for_dir_tree(const std::string &dir_path, T apply, bool recursive = true) {
        ...
        if(recursive) {
            for(auto const &entry: std::filesystem::recursive_directory_iterator{p}) {
                ...
                std::stringstream ss{};
                ss << tt << "+0";
                dfi::timespec_wrapper mt{ss.str()};
#else
                dfi::timespec_wrapper mt{(static_cast<long double>(tt.time_since_epoch().count()) / 1'000'000'000.0L) + 6437664000.0L};
#endif
