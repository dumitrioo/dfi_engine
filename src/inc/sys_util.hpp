...
#define HOST_ORDER (dfi::sys_util::host_order.value)
#define HOST_ORDER_LE (dfi::sys_util::HOST_ORDER_LITTLE_ENDIAN)
#define HOST_ORDER_BE (dfi::sys_util::HOST_ORDER_BIG_ENDIAN)
#define HOST_ORDER_PDP (dfi::sys_util::HOST_ORDER_PDP_ENDIAN)
        ...
        void for_reg_key(HKEY hk,
                        const dfi::str_util::tstring &path,
                        FUNC_T apply,
        ...
        static std::string backtrace() {
            ...
            while(unw_step(&cursor) > 0) {
                ...
                ss << "0x" << dfi::str_util::utoa<std::string>(pc, 16);
                ...
                if(unw_get_proc_name(&cursor, sym, sizeof(sym), &offset) == 0) {
                    ...
                    ss << ' ' << sym_str << "+0x" << dfi::str_util::utoa<std::string>(offset, 16) << '\n';
                } else {
        ...
        static std::string get_exec_path() {
            ...
            if(readlink("/proc/self/exe", path, dest_len) != -1) {
                return dfi::file_util::extract_file_dir(path);
            }
            return path;
#elif defined(PLATFORM_WINDOWS)
...
#ifdef UNICODE
            return dfi::str_util::to_utf8(std::wstring{szFileName});
#else
            return szFileName;
#endif
#else
        ...
        static std::string system_vendor() {
            std::string res{};
#if defined(PLATFORM_LINUX) || defined(PLATFORM_ANDROID)
            auto vendor_info{dfi::file_util::load_from_file("/sys/devices/virtual/dmi/id/sys_vendor")};
            res = dfi::str_util::trim(std::string{vendor_info.begin(), vendor_info.end()});
#endif
            return res;
        }
            ...
            if(mounts_file) {
                ...
                while(nullptr != (ent = getmntent(mounts_file))) {
                    if(std::string{ent->mnt_dir} == "/") {
                        root_fs = dfi::file_util::extract_file_name(ent->mnt_fsname);
                        found = true;
                        break;
                    }
                }
                ...
                if(found) {
                    for(
                        ...
                        if(std::filesystem::is_symlink(it->symlink_status())) {
                            std::string fname{dfi::file_util::extract_file_name(it->path())};
                            std::string slnk{dfi::file_util::extract_file_name(std::filesystem::read_symlink(*it))};
                            if(slnk == root_fs) {
                                res = fname;
                                break;
                            }
