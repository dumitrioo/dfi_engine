...
namespace dfi {
    ...
    private:
        ...
        static bool valid_indexes(std::vector<dfi::fsm_tokenizer<std::wstring>::token> const &tokens, int begin_indx, int end_indx) {
            return begin_indx >= 0 && begin_indx < (int)tokens.size() && end_indx >= 0 && end_indx < (int)tokens.size() && begin_indx <= end_indx;
        }

        std::shared_ptr<node> parse_most_right_num(std::vector<dfi::fsm_tokenizer<std::wstring>::token> const &tokens, int begin_indx, int end_indx) {
            if(!valid_indexes(tokens, begin_indx, end_indx)) {
                return {};
            }
            std::shared_ptr<node> res{std::make_shared<node>()};
            int new_end{end_indx - 1};
            if(dfi::is_first_one_of_the_rest(tokens[end_indx].type, L"int", L"fpl")) {
                res->kind = node_kind::num_val;
                res->number = std::atof(dfi::str_util::to_utf8(tokens[end_indx].value).c_str());

                bool sgn_done{false};
                if(new_end >= begin_indx) {
                    if(dfi::is_first_one_of_the_rest(tokens[new_end].type, L"minus", L"plus")) {
                        if(tokens[new_end].type == L"minus") { res->number = -res->number; }
                        new_end--;
                        sgn_done = true;
                    } else if(dfi::is_first_one_of_the_rest(tokens[new_end].type, L"spc")) {
                        new_end--;
                    }
                }
        ...
        std::shared_ptr<node> parse_dms_sym(std::vector<dfi::fsm_tokenizer<std::wstring>::token> const &tokens, int begin_indx, int end_indx) {
            if(!valid_indexes(tokens, begin_indx, end_indx)) {
                return {};
            }
            std::shared_ptr<node> res{std::make_shared<node>()};
            ...
            bool is_cur_num{dfi::is_first_one_of_the_rest(tokens[end_indx].type, L"fpl", L"int")};
        ...
        std::shared_ptr<node> parse_height(std::vector<dfi::fsm_tokenizer<std::wstring>::token> const &tokens, int begin_indx, int end_indx) {
            while(
                begin_indx < (int)tokens.size() && begin_indx <= end_indx &&
                !dfi::is_first_one_of_the_rest(tokens[begin_indx].type, L"int", L"fpl", L"plus", L"minus")
            ) {
                ++begin_indx;
            }
            if(!valid_indexes(tokens, begin_indx, end_indx)) {
        ...
        std::shared_ptr<node> parse_coord(std::vector<dfi::fsm_tokenizer<std::wstring>::token> const &tokens, int begin_indx, int end_indx) {
            while(
                begin_indx < (int)tokens.size() && begin_indx <= end_indx &&
                !dfi::is_first_one_of_the_rest(tokens[begin_indx].type, L"int", L"fpl", L"plus", L"minus")
            ) {
            ...
            if(wsides.size() == 1) {
                ...
                res->kind = dfi::is_first_one_of_the_rest(res->wrldside, world_side::north, world_side::south) ? node_kind::coord_lat : node_kind::coord_lon;
        ...
        void parse_coords(std::vector<dfi::fsm_tokenizer<std::wstring>::token> const &tokens) {
            if(tokens.size() == 0) {
                return;
            }
            int begin_indx{0};
            ...
            for(int i{begin_indx}; i <= end_indx; ++i) {
                ...
                } else if(dfi::is_first_one_of_the_rest(tokens[i].type, L"N", L"S")) {
                    lat_lon_indexes.push_back(i);
                } else if(dfi::is_first_one_of_the_rest(tokens[i].type, L"W", L"E")) {
                    lat_lon_indexes.push_back(i);
                } else if(tokens[i].type == L"m") {
                    height_indexes.push_back(i);
                } else if(dfi::is_first_one_of_the_rest(tokens[i].type, L"plus", L"minus")) {
                    signs_indexes.push_back(i);
                } else if(dfi::is_first_one_of_the_rest(tokens[i].type, L"int")) {
                    nums_indexes.push_back(i);
                    int_nums_indexes.push_back(i);
                } else if(dfi::is_first_one_of_the_rest(tokens[i].type, L"fpl")) {
                    nums_indexes.push_back(i);
            ...
            do {
                if(!lat_lon_indexes.empty()) {
                    ...
                    if(dfi::is_first_one_of_the_rest(tokens[lat_lon_indexes[0]].type, L"E", L"W")) {
        ...
        void parse(std::string const &coords) {
            std::wstring const wcoords{dfi::str_util::from_utf8(coords)};
            std::vector<dfi::fsm_tokenizer<std::wstring>::token> tokens{};
            dfi::fsm_tokenizer<std::wstring> t{
                L"#rd|[:digit:]>>int;"
                ...
                L"#rd|$df>>^sym;"
                ,
                [&](dfi::fsm_tokenizer<std::wstring>::token const &tkn) { tokens.push_back(tkn); }
            };
...
static std::ostream &operator<<(std::ostream &os, dfi::LLA const &v) {
    os << v.to_string_traditional(3);
    return os;
}

static std::ostream &operator<<(std::ostream &os, dfi::ECEF const &v) {
    std::stringstream ss{};
...
static std::ostream &operator<<(std::ostream &os, dfi::ENU const &v) {
    std::stringstream ss{};
