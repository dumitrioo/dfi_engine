    ...
    public:
        ...
        void start(
            ...
            if(!started_) {
                ...
                lsnr_ = std::make_unique<dfi::net::socket>();
                ...
                poller_ = std::make_unique<dfi::net::socket_poller>();
                if(!poller_->add_event(lsnr_->handle(), dfi::net::POLL_EVENT_IN | dfi::net::POLL_EVENT_ET)) {
                    throw_errno(errno);
                }
                cq_->enqueue(fn_);
                started_ = true;
        ...
        void start(
            QUEUE_T *cq,
            std::string const &bind_address, int port, int wait_events_count,
            bool enable_ssl, dfi::bytevec priv, dfi::bytevec pub
            ) {
            std::unique_lock l{started_mtp_};
            if(!started_) {
                if(!cq) {
        ...
        void set_ssl_enabled(bool
            ...
            if(val) {
                ssl_ctx_ = std::make_unique<dfi::ssl::context>(true);
                ssl_ctx_->init();
            } else {
                ssl_ctx_.reset();
            }
        ...
        bool set_keys(std::string
            ...
            std::unique_lock l{ssl_mtp_};
            try {
                if(ssl_ctx_ && dfi::file_util::file_exists(priv_file_name) && dfi::file_util::file_exists(pub_file_name)) {
        ...
        int send(conn_id_t id, dfi::bytevec const &data, long double timeout_seconds = 15) {
            return send(id, data.data(), data.size(), timeout_seconds);
        }

        std::optional<dfi::bytevec> get_conn_data(conn_id_t id) {
            std::shared_ptr<connection> conn{get_conn_by_id(id)};
            dfi::bytevec bv{};
        ...
        void send_async(conn_id_t id, void const *data, std::int32_t data_size, std::function<void(int)> send_callback = nullptr, long double timeout_seconds = 15) {
            cq_->enqueue([&]() {
                uses_++;
                dfi::shut_on_destroy at_exit{[&]() { uses_--; }};
                std::shared_ptr<connection> conn{get_conn_by_id(id)};
                if(conn) {
    ...
    private:
        std::function<void()> fn_{
            [&]() {
                ...
                if(!termination()) {
                    try {
                        std::vector<dfi::net::poll_event> events{};
                        {
                            std::shared_lock l{poller_mtp_};
                            if(poller_.get() != nullptr) {
                                events = poller_->wait(wait_events_count_, dfi::timespec_wrapper{0.25});
                            } else {
                                l.unlock();
        ...
        public:
            ...
            void push_buff_data(dfi::bytevec const &buf) {
                std::unique_lock l{chunks_buffer_mtp_};
                chunks_buffer_.push_back(buf);
            }

            void push_buff_data(dfi::bytevec &&buf) {
                std::unique_lock l{chunks_buffer_mtp_};
            ...
            bool pop_buff_data(dfi::bytevec &bv) {
                std::unique_lock l{chunks_buffer_mtp_};
                size_t total{0};
            ...
            std::shared_ptr<dfi::net::socket> sckt_{std::make_shared<dfi::net::socket>()};
#ifdef DFI_TCPSERVER_USE_SSL
            std::shared_ptr<dfi::ssl::connection> ssl_conn_{nullptr};
#endif
            mutable std::shared_mutex chunks_buffer_mtp_{};
            std::list<dfi::bytevec> chunks_buffer_{};
            bool doomed_{false};
        };
                ...
                try {
                    while(true) {
                        ...
                        if(new_conn) {
                            ...
                            ins_conn(new_conn);
                            std::shared_lock l{poller_mtp_};
                            if(!poller_->add_event(new_conn->sckt_->handle(), dfi::net::POLL_EVENT_IN | dfi::net::POLL_EVENT_ET)) {
                                l.unlock();
                                rm_conn(new_conn);
                                new_conn.reset();
                            } else {
        ...
        void process_poll_event(dfi::net::poll_event curr_evt) {
            if(termination()) {
                return;
            }
            uses_++;
            cq_->enqueue([this, curr_evt]() {
                ...
                if(curr_conn) {
                    ...
                    if(curr_evt.events & dfi::net::POLL_EVENT_IN) {
                        bool ok{true};
                        size_t numtotal{0};
                        while(true) {
                            try {
        ...
        QUEUE_T *cq_{};
        dfi::atomic_sequence_generator<conn_id_t> conn_id_gen_{1};
        ...
        std::unique_ptr<dfi::net::socket_poller> poller_{};
        std::shared_mutex lsnr_mtp_{};
        std::unique_ptr<dfi::net::socket> lsnr_{};
        std::shared_mutex ssl_mtp_{};
#ifdef DFI_TCPSERVER_USE_SSL
        std::unique_ptr<dfi::ssl::context> ssl_ctx_{nullptr};
#endif
