...
namespace dfi::net {
    ...
    public:
        ...
        bool bind(const std::string &address, std::uint16_t port) {
            if(ok()) {
                if(sock_type_ == address_family::inet4) {
                    ...
                    common_addr_.sock_addr_.sin_port = dfi::bit_util::hnswap<std::uint16_t>{static_cast<std::uint16_t>(port)}.val;
                    common_addr_.sock_addr_.sin_addr = dfi::net::pton(address);
                    ...
                    common_addr_.sock_addr6_.sin6_port = dfi::bit_util::hnswap<std::uint16_t>{static_cast<std::uint16_t>(port)}.val;
                    common_addr_.sock_addr6_.sin6_addr = dfi::net::pton6(address);
        ...
        void accept(socket &client_sock) {
            if(ok()) {
                ...
                if(client_sock.sock_fd_ == -1) {
                    ...
                    throw socket_error{dfi::sys_util::error_str(dfi::sys_util::last_error())};
                }
            } else {
        ...
        bool connect(const std::string &address, std::uint16_t port) {
            if(ok()) {
                if(sock_type_ == address_family::inet4) {
                    auto ipaddr{dfi::net::resolve(address)};
                    ...
                    sock_addr.sin_port = dfi::bit_util::hnswap<std::uint16_t>{static_cast<std::uint16_t>(port)}.val;
                    sock_addr.sin_addr = dfi::net::pton(add);
                    int conn_res{::connect(sock_fd_, (struct sockaddr *)&sock_addr, sizeof(struct sockaddr))};
                    if(conn_res == 0) {
                        return true;
                    }
                } else if(sock_type_ == address_family::inet6) {
                    auto ipaddr{ dfi::net::resolve6(address) };
                    std::string add{ntop(ipaddr)};
                    if(add.empty()) {
                        return false;
                    }
                    ...
                    sock_addr.sin6_port = dfi::bit_util::hnswap<std::uint16_t>{static_cast<std::uint16_t>(port)}.val;
                    sock_addr.sin6_addr = dfi::net::pton6(add);
                    int conn_res{::connect(sock_fd_, (struct sockaddr *)&sock_addr, sizeof(struct sockaddr))};
                    if(conn_res == 0) {
                        return true;
                    }
        ...
        bool set_rcv_timeout(dfi::timespec_wrapper const &to, dfi::timespec_wrapper &orig_to) {
            bool result{false};
            if(sock_fd_ != -1) {
                ...
                if(::getsockopt(sock_fd_, SOL_SOCKET, SO_RCVTIMEO, &org_tv, &org_tv_len) == 0) {
                    orig_to = dfi::timespec_wrapper{(long double)org_tv.tv_sec + (long double)org_tv.tv_usec / 1'000'000.0L};
        ...
        int send_message(const void *data, std::uint32_t data_size) {
            int res{};
            if(data && data_size > 0) {
                std::uint32_t net_size = dfi::bit_util::hnswap<std::uint32_t>{data_size}.val;
                ...
                do {
                    int wrote_count = dfi::net::socket::send(curr_buff + total_sent, total_size - total_sent);
                    if(wrote_count < 0) {
                        int e{static_cast<int>(dfi::sys_util::last_error())};
                        while(e == EAGAIN) {
                            wrote_count = dfi::net::socket::send(curr_buff + total_sent, total_size - total_sent);
                            if(wrote_count < 0) {
                                e = dfi::sys_util::last_error();
                            } else {
                                e = 0;
                            }
                        }
    ...
    std::vector<std::uint8_t> receive_message() {
        ...
        if(ok()) {
            ...
            if(msg_size_vec.size() == sizeof(std::uint32_t)) {
                std::uint32_t msg_size{dfi::bit_util::from_net_bytes<std::uint32_t>(msg_size_vec.data())};
                std::uint32_t msg_size_host{dfi::bit_util::hnswap<std::uint32_t>{msg_size}.val};
                for(result = dfi::net::socket::receive(msg_size_host); result.size() < msg_size_host; ) {
                    std::vector<std::uint8_t> data_partial_vec{dfi::net::socket::receive(msg_size_host - result.size())};
