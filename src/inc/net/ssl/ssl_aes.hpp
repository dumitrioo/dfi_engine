    ...
    namespace ssl {
        ...
        public:
            ...
            aes_crydec(std::string const &key, std::string const &iv):
                key_{dfi::ssl::hash::sha3_256(key)}, iv_{dfi::ssl::hash::sha3_128(iv)}
            {}

            aes_crydec(std::string const &key):
                key_{dfi::ssl::hash::sha3_256(key)}, iv_{dfi::ssl::hash::sha3_128(key)}
            {}
        ...
        private:
            std::vector<std::uint8_t> key_{dfi::crypt::gen_rand_bytes(KEY_SIZE)};
            std::vector<std::uint8_t> iv_{dfi::crypt::gen_rand_bytes(BLOCK_SIZE)};
            dfi::ssl::unique_ptr<EVP_CIPHER_CTX> ctx_{dfi::ssl::NEW<EVP_CIPHER_CTX>([]() { return EVP_CIPHER_CTX_new(); })};
        };
