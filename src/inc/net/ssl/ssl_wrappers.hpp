...
namespace dfi {
    ...
    namespace ssl {
        ...
        public:
            ...
            connection &init(dfi::ssl::unique_ptr<SSL> &&ssl_conn, std::shared_ptr<net::socket> skt, std::string const &ca_cert, action act = NONE) {
                ok_ = false;
        ...
        std::int64_t interruptible_send_message(std::vector<std::uint8_t> const &data_for_out) {
            ...
            size_buffer_ = dfi::bit_util::hnswap<std::uint32_t>{data_for_out.size()}.val;
            size_buffer_sent_ = 0;
            output_buffer_ = {};
            output_buffer_sent_ = 0;
        ...
        private:
            ...
            void connect_unlocked(std::string const &ca_cert) {
                ...
                if(!ca_cert.empty()) {
                    dfi::ssl::unique_ptr<X509> server_cert{SSL_get_peer_certificate(ssl_conn_ptr_.get())};
            ...
            bool verify_peer_cert(std::string const &ca_cert_data, X509 *cert_to_check) {
              dfi::ssl::unique_ptr<X509_STORE> store{dfi::ssl::NEW<X509_STORE>([](){ return X509_STORE_new(); })};
              if (!store) {
                  return false;
              }
              dfi::ssl::unique_ptr<X509_STORE_CTX> vrfy_ctx{dfi::ssl::NEW<X509_STORE_CTX>([](){ return X509_STORE_CTX_new(); })};
              dfi::ssl::unique_ptr<BIO> cbio{dfi::ssl::NEW<BIO>([&]() { return BIO_new_mem_buf(ca_cert_data.data(), ca_cert_data.size()); })};
              dfi::ssl::unique_ptr<X509> ca_crt{dfi::ssl::NEW<X509>([&]() { return PEM_read_bio_X509(cbio.get(), NULL, 0, NULL); })};
            ...
            dfi::ssl::unique_ptr<SSL> ssl_conn_ptr_{nullptr};
            bool ok_{false};
        ...
        public:
            ...
            void init(ssl_method m = TLS) {
                ...
                if(server_) {
                    ...
                    ctx_ = dfi::ssl::NEW<SSL_CTX>([&]() { return SSL_CTX_new(mthd); });
            ...
            void add_cert(const std::vector<std::uint8_t> &cr) {
                ...
                dfi::ssl::unique_ptr<BIO> cbio{dfi::ssl::NEW<BIO>([&]() { return BIO_new_mem_buf(cr.data(), -1); })};
                dfi::ssl::unique_ptr<X509> cert{dfi::ssl::NEW<X509>([&]() { return PEM_read_bio_X509(cbio.get(), nullptr, 0, nullptr); })};
                if(!cert) {
                    throw_last_error();
                }
            ...
            void set_server_keys(const std::vector<std::uint8_t> &cr, const std::vector<std::uint8_t> &pk) {
                ...
                dfi::ssl::unique_ptr<BIO> kbio{dfi::ssl::NEW<BIO>([&]() { return BIO_new_mem_buf(pk.data(), -1); })};
                dfi::ssl::unique_ptr<RSA> rsa{dfi::ssl::NEW<RSA>([&]() { return PEM_read_bio_RSAPrivateKey(kbio.get(), nullptr, 0, nullptr); })};
                if(!rsa) {
                    throw_last_error();
                }
            ...
            std::shared_ptr<connection> get_new_connection(std::shared_ptr<net::socket> sk, std::string const &ca_cert) {
                ...
                dfi::ssl::unique_ptr<SSL> ssl_cnn{dfi::ssl::NEW<SSL>([&]() { return SSL_new(ctx_.get()); })};
        ...
        private:
            dfi::ssl::unique_ptr<SSL_CTX> ctx_{};
            bool server_{false};
        };
