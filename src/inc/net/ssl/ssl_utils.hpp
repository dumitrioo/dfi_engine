    ...
    namespace ssl {
        ...
        static std::pair<std::string, std::string> generate_rsa_key_pair(int bits = 4096) {
            try {
                ...
                ssl_ptr<BIGNUM> bne{dfi::ssl::NEW<BIGNUM>([&]() { return BN_new(); })};
                res = BN_set_word(bne.get(), e) == 1;
                if(!res) { throw_last_error(); }
                ssl_ptr<RSA> r{dfi::ssl::NEW<RSA>([&]() { return RSA_new(); })};
                res = RSA_generate_key_ex(r.get(), bits, bne.get(), nullptr) == 1;
                if(!res) { throw_last_error(); }
                ssl_ptr<BIO> bp_public{dfi::ssl::NEW<BIO>([&]() { return BIO_new(BIO_s_mem()); })};
                res = PEM_write_bio_RSAPublicKey(bp_public.get(), r.get()) == 1;
                if(!res) { throw_last_error(); }
                ssl_ptr<BIO> bp_private{dfi::ssl::NEW<BIO>([&]() { return BIO_new(BIO_s_mem()); })};
        ...
        static std::vector<std::uint8_t> rsa_encrypt(std::vector<std::uint8_t> const &plain_data, std::string const &pubkey) {
            ...
            ssl_ptr<BIO> rsaPublicBIO{dfi::ssl::NEW<BIO>([&]() { return BIO_new_mem_buf(rsaPublicKeyChar, -1); })};
            RSA *rsaPublicKey = nullptr;
            ssl_ptr<RSA> rsa_pub_read{dfi::ssl::NEW<RSA>([&]() { return PEM_read_bio_RSAPublicKey(rsaPublicBIO.get(), &rsaPublicKey, 0, nullptr); })};
            if(rsa_pub_read) {
                int encrypt_len{-1};
        ...
        static std::vector<std::uint8_t> rsa_decrypt(std::vector<std::uint8_t> const &enc_data, std::string const &privkey) {
            ...
            ssl_ptr<BIO> rsaPrivateBIO{dfi::ssl::NEW<BIO>([&]() { return BIO_new_mem_buf(rsaPrivateKeyChar, -1); })};
            RSA *rsaPrivateKey = nullptr;
            unique_ptr<RSA> rsa_priv_read{dfi::ssl::NEW<RSA>([&]() { return PEM_read_bio_RSAPrivateKey(rsaPrivateBIO.get(), &rsaPrivateKey, nullptr, nullptr); })};
            if(rsa_priv_read) {
    ...
    //            ecc                                                                                                  //
        ...
        static std::vector<std::uint8_t> prepend_zeroes(std::vector<std::uint8_t> const &v, std::size_t const new_size) {
            ...
            if(res.size() < new_size) {
                dfi::bit_util::inplace_swap(&res[0], res.size());
        ...
        static std::vector<std::uint8_t> EC_MV_decrypt(std::vector<std::uint8_t> const &cipher, EC_KEY *privkey) {
            ...
            int digsiz{0};
            {
                dfi::serial_reader cipher_serial{&cipher[0], cipher.size()};
                dfi::serial_reader::const_iterator it{cipher_serial.begin()};
                {
        ...
        static bool check_ec_pair_match(std::string const &pri, std::string const &pub) {
            ...
            try {
                ...
                std::vector<std::uint8_t> ctrl{dfi::ssl::EC_MV_encrypt(bytes, pub)};
                std::vector<std::uint8_t> ucontrol{dfi::ssl::EC_MV_decrypt(ctrl, pri)};
                res = bytes == ucontrol;
            } catch(...) {
            }
            return res;
        ...
        static std::vector<std::uint8_t> sign_buff_new(std::string const &priv_key_pem, unsigned char const *buff, int buff_len) {
            ...
            if(is_valid_priv_key(priv_key_pem) && buff && buff_len > 0) {
                ...
                auto pri{dfi::ssl::priv_pem_to_ec_key(priv_key_pem)};
                ssl_ptr<EVP_PKEY> key{EVP_PKEY_new()};
                if(!key) { dfi::ssl::throw_last_error_if_nonzero(); }
                if(EVP_PKEY_set1_EC_KEY(key.get(), pri.get()) <= 0) { dfi::ssl::throw_last_error_if_nonzero(); }
                ssl_ptr<EVP_PKEY_CTX> key_ctx{EVP_PKEY_CTX_new(key.get(), nullptr)};
                if(!key_ctx) { dfi::ssl::throw_last_error_if_nonzero(); }
                if(EVP_PKEY_sign_init(key_ctx.get()) <= 0) { dfi::ssl::throw_last_error_if_nonzero(); }
                if(EVP_PKEY_CTX_set_signature_md(key_ctx.get(), EVP_sha3_256()) <= 0) { dfi::ssl::throw_last_error_if_nonzero(); }

                std::size_t sig_len{};
                if(EVP_PKEY_sign(key_ctx.get(), nullptr, &sig_len, md.data(), md.size()) <= 0) { dfi::ssl::throw_last_error_if_nonzero(); }
                unsigned char *sig{(unsigned char *)OPENSSL_malloc(sig_len)};
                if(!sig) { dfi::ssl::throw_last_error_if_nonzero(); }
                dfi::shut_on_destroy freemem{[&]() { OPENSSL_free(sig); }};

                if(EVP_PKEY_sign(key_ctx.get(), (unsigned char *)&sig[0], &sig_len, md.data(), md.size()) <= 0) { dfi::ssl::throw_last_error_if_nonzero(); }
        ...
        static bool verify_sig_of_buff_new(std::string const &pub_key_pem, unsigned char const *buff, std::size_t buff_len, std::vector<std::uint8_t> const &sig) {
            ...
            if(buff && buff_len && sig.size()) {
                auto pub{dfi::ssl::pub_pem_to_ec_key(pub_key_pem)};
                ssl_ptr<EVP_PKEY> key{EVP_PKEY_new()};
                if(!key) {
                    dfi::ssl::throw_last_error_if_nonzero();
                }
        ...
        static std::vector<std::uint8_t> encrypt_signed_new(std::vector<std::uint8_t> const &plaintext, std::vector<std::uint8_t> const &candidate_key, std::string const &authors_priv_pem) {
            try {
                auto ph{dfi::crypt::sha3_512_compute(plaintext)};
                std::vector<std::uint8_t> sig{dfi::ssl::sign_buff_new(authors_priv_pem, ph)};
                if(!sig.empty()) {
                    std::vector<std::uint8_t> cipher{};
                    ...
                    if(key.size() != 32) {
                        auto k256{dfi::crypt::sha3_256_compute(key)};
                        key.assign(k256.begin(), k256.end());
                    }
                    if(dfi::crypt::chacha20_poly1305::encrypt(key.data(), key.size(), iv.data(), iv.size(), sig.data(), sig.size(), (const uint8_t *)plaintext.data(), &cipher[0], plaintext.size(), &mac[0], mac.size())) {
                        dfi::serializer sr{};
                        sr << sig << cipher << mac << iv;
                        return dfi::crypt::block_container_encrypt(sr.data_vec(), candidate_key);
                    }
                }
            } catch(...) {
            }
            ...
            try {
                ...
                if(!sig.empty()) {
                    ...
                    if(dfi::crypt::chacha20_poly1305::encrypt(key.data(), key.size(), iv.data(), iv.size(), sig.data(), sig.size(), (const uint8_t *)plaintext.data(), &cipher[0], plaintext.size(), &mac[0], mac.size())) {
                        dfi::serializer sr{};
                        sr << sig << cipher << mac << iv;
                        return dfi::data_to_base64_str(dfi::crypt::block_container_encrypt(sr.data_vec(), candidate_key));
                    }
                }
            } catch(...) {
            }
        ...
        static std::string decrypt_signed_new(std::string const &b64_cipher, std::vector<std::uint8_t> const &candidate_key, std::string const &authors_pub_pem, bool *is_success = nullptr) {
            ...
            try {
                ...
                std::vector<std::uint8_t> cipher_pack{dfi::crypt::block_container_decrypt(dfi::base64_str_to_data(b64_cipher), candidate_key)};
                if(cipher_pack.empty()) { return {}; }
                dfi::serial_reader rd{cipher_pack.data(),cipher_pack.size()};
                ...
                if(dfi::crypt::chacha20_poly1305::decrypt(key.data(), key.size(), iv.data(), iv.size(), sig.data(), sig.size(), (const uint8_t *)cipher.data(), &plaintext[0], cipher.size(), &mac[0], mac.size())) {
                    auto ph{dfi::crypt::sha3_512_compute(plaintext)};
                    if(dfi::ssl::verify_sig_of_buff_new(authors_pub_pem, (const unsigned char *)ph.data(), ph.size(), sig)) {
        ...
        static std::vector<std::uint8_t> decrypt_signed_new(std::vector<std::uint8_t> const &ciphertext, std::vector<std::uint8_t> const &candidate_key, std::string const &authors_pub_pem, bool *is_success = nullptr) {
            ...
            try {
                ...
                std::vector<std::uint8_t> cipher_pack{dfi::crypt::block_container_decrypt(ciphertext, candidate_key)};
                if(cipher_pack.empty()) { return {}; }
                dfi::serial_reader rd{cipher_pack.data(),cipher_pack.size()};
                auto it{rd.begin()}; if(!it.valid()) { return {}; }
                std::vector<std::uint8_t> sig{it->as_bytevec()}; ++it; if(!it.valid()) { return {}; }
