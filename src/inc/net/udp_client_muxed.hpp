...
namespace dfi::net {
    ...
    public:
        ...
        void set_local_key(dfi::bytevec const &key, dfi::bytevec const &iv) {
            crypto_ctr_ = 0;
            out_gamma_.init(key.data(), key.size(), iv.data(), iv.size());
        }

        void set_remote_key(dfi::bytevec const &key, dfi::bytevec const &iv) {
            ...
            if(sock_fd_ != -1) {
                ...
                if(sock_type_ == address_family::inet4) {
                    if((sock_fd_ = ::socket(AF_INET, SOCK_DGRAM, 0)) != -1) {
                        in_addr dst_ip{dfi::net::resolve(addr)};
                    ...
                    if((sock_fd_ = ::socket(AF_INET6, SOCK_DGRAM, 0)) != -1) {
                        in6_addr dst_ip{dfi::net::resolve6(addr)};
        ...
        //             }
        //             if(sock_fd_ != -1 && n > 0) {
        //                 dfi::serial_reader sr{buffer.data(), (std::size_t)n};
        //                 dfi::serial_reader::const_iterator it{sr.cbegin()};
        //                 if(it->as_string() == "ok") {
        //                     ++it;
            ...
            if(sock_fd_ >= 0) {
                ...
                if(n == 5 && std::memcmp("close", buffer.data(), 5) == 0) {
                    ...
                    if(crc_orig == crc_calc.val) {
                        ...
                        if(auto demuxed{demuxer_.add_data(buffer.data() + 8, (dfi::serial_reader::size_type)n - 8)}) {
                            dfi::serial_reader const ser{demuxed->data(), (dfi::serial_reader::size_type)demuxed->size()};
                            dfi::serial_reader::const_iterator iter{ser.cbegin()};
    ...
    private:
        ...
        std::vector<std::uint8_t> encrypt_data(std::vector<std::uint8_t> const &p, std::uint64_t ctr_start) {
            ...
            dfi::crypt::gamma out_gamma{out_gamma_};
            lck.unlock();
        ...
        std::vector<std::uint8_t> decrypt_data(void const *e, std::size_t es, std::uint64_t ctr_start) {
            ...
            dfi::crypt::gamma in_gamma{in_gamma_};
            lck.unlock();
                            ...
                            if(sock_fd_ >= 0) {
                                ...
                                if(n == 5 && std::memcmp("close", buffer.data(), 5) == 0) {
                                    ...
                                    if(crc_orig == crc_calc.val) {
                                        std::unique_lock l{demuxer_mtp_};
                                        if(auto demuxed{demuxer_.add_data(buffer.data() + 8, (dfi::serial_reader::size_type)n - 8)}) {
                                            dfi::serial_reader const ser{
                                                demuxed->data(), (dfi::serial_reader::size_type)demuxed->size()
                                            };
                                            dfi::serial_reader::const_iterator iter{ser.cbegin()};
                                            if(iter->as_unumber() == 0) {
                                                ++iter;
    ...
    private:
        ...
        dfi::net::packets_muxer<std::uint16_t> muxer_{NET_PACKET_PAYLOAD_SIZE_MAX};
        std::mutex demuxer_mtp_{};
        dfi::net::packets_demuxer demuxer_{};
        ...
        dfi::crypt::gamma out_gamma_{};
        dfi::crypt::gamma in_gamma_{};
        crc64 crc_chkr_{};
