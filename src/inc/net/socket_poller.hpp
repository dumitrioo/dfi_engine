...
namespace dfi::net {
    ...
    namespace impl {
        ...
        public:
            ...
            std::vector<poll_event> wait(std::size_t maxcount = 64, dfi::timespec_wrapper const &timeout = dfi::eternity) {
                ...
                if(!events_vec.empty()) {
                    int poll_res = ::poll(&events_vec[0], events_vec.size(), timeout == dfi::eternity ? std::numeric_limits<int>::max() : timeout.milliseconds());
                    if(poll_res > 0) {
                        ...
                        last_error_ = dfi::sys_util::last_error();
                        throw poll_error{dfi::sys_util::error_str(last_error_)};
                    }
                }
                return result;
            }
        ...
        public:
            ...
            std::vector<poll_event> wait(std::size_t maxcount = 64, dfi::timespec_wrapper const &timeout = dfi::eternity) {
                ...
                int wait_result = ::epoll_wait(epfd_, &events_buf[0], maxcount, timeout == dfi::eternity ? -1 : timeout.milliseconds());
                if(wait_result > 0) {
                    ...
                    if (dfi::sys_util::last_error() != EINTR) {
                        last_error_ = dfi::sys_util::last_error();
                        throw poll_error{dfi::sys_util::error_str(dfi::sys_util::last_error())};
                    }
                }
