...
namespace dfi::net {
    ...
    static std::string ntop(in_addr_t i) {
        std::string res{};
        in_addr a{};
        a.s_addr = dfi::bit_util::swap_on_le<in_addr_t>{i}.val;
        std::vector<char> buff{}; buff.resize(512);
        auto ret{inet_ntop(AF_INET, &a, &buff[0], buff.size())};
        if(ret) {
            res = ret;
    ...
    static std::string ipv4_to_hex(std::string const &addr) {
        in_addr an{dfi::net::pton(addr)};
        std::string res{dfi::str_util::utoa<std::string>(dfi::bit_util::swap_on_le<std::uint32_t>{an.s_addr}.val, 16)};
        while(res.size() < 8) {
    ...
    static std::string ipv6_to_hex(std::string const &addr) {
        std::string res{};
        in6_addr an{dfi::net::pton6(addr)};
        for(int i{0}; i < 16; ++i) {
            std::string ns{dfi::str_util::utoa<std::string>(((std::uint8_t const *)&an)[i], 16)};
            while(ns.size() < 2) {
                ns = std::string{"0"} + ns;
            }
            res += ns;
