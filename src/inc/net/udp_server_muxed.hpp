...
namespace dfi::net {
    ...
    public:
        ...
        void set_on_data_arrived(std::function<void(std::uint64_t, void const *, std::size_t)> const &on_data_from_client) {
            std::unique_lock l{on_data_from_client_mtp_};
        ...
        void wait() {
            while(start_.load(std::memory_order_relaxed) || running_.load(std::memory_order_relaxed)) {
                std::this_thread::sleep_for(std::chrono::milliseconds{10});
            }
        }

        bool started() const {
            std::shared_lock ifl{mtp_};
        ...
        bool remove_conn(std::uint64_t conn_id) {
            ...
            if(it != connections_by_id_.end()) {
                ...
                notify_connection_closed(conn_id);
                res = true;
            }
            return res;
        }
                        ...
                        if(crc_calc.val == crc_orig) {
                            ...
                            if(msg) {
                                ...
                                if(iter->as_unumber() == 0) {
                                    ++iter;
                                    notify_data_from_client(conn_ptr->conn_id(), iter->data(), iter->size());
                                } else {
                                    ++iter;
                                    std::uint64_t ctr_start{iter->as_unumber()};
                                    ++iter;
                                    std::vector<std::uint8_t> d{conn_ptr->decrypt_data(iter->data(), iter->size(), ctr_start)};
                                    notify_data_from_client(conn_ptr->conn_id(), d.data(), d.size());
                                }
                            }
                        }
                    }
    ...
    private:
        ...
        void notify_new_connection(std::uint64_t cid) {
            std::shared_lock l{on_new_connection_mtp_};
            if(on_new_connection_) { on_new_connection_(cid); }
        }

        void notify_connection_closed(std::uint64_t cid) {
            std::shared_lock l{on_connection_closed_mtp_};
            if(on_connection_closed_) { on_connection_closed_(cid); }
        }

        void notify_data_from_client(std::uint64_t cid, void const *d, std::size_t s) {
            std::shared_lock l{on_data_from_client_mtp_};
            if(on_data_from_client_) { on_data_from_client_(cid, d, s); }
        }
    ...
    private:
        ...
        mutable std::shared_mutex on_data_from_client_mtp_{};
