...
namespace dfi {
                        ...
                        if(state >= 0 && state < fi) {
                            ...
                            if(!std::isdigit(currc)) {
                                if(state == ye && !sign_set && (currc == '-' || currc == '+')) {
                                    ...
                                    if(!buf.empty()) {
                                        brdn[state] = dfi::str_util::atoi(std::string{buf});
                                        buf.clear();
                                ...
                                if(cno + 1 == dts.size() && !buf.empty()) {
                                    std::int64_t bval{dfi::str_util::atoi(std::string{buf})};
                                    brdn[state] = bval;
                                }
                            }
                        } else {
    ...
    private:
        static std::string subseconds_to_str(long double val, std::size_t prec) {
            if(prec > 0) {
                if(prec > 9) { prec = 9; }
                std::string sscnd{dfi::str_util::ftoa(val, prec)};
        ...
        void parse_iso_8601(CHAR_T const *buff, std::size_t bsize) {
            try {
                ...
                dfi::static_buff<int, 24> num_idx{};
                dfi::static_buff<int, 24> colon_idx{};
                dfi::static_buff<int, 24> dot_idx{};
                dfi::static_buff<int, 24> comma_idx{};
                dfi::static_buff<int, 24> minus_idx{};
                dfi::static_buff<int, 24> plus_idx{};
                dfi::static_buff<int, 24> t_idx{};
                dfi::static_buff<int, 24> w_idx{};
                dfi::static_buff<int, 24> z_idx{};
                dfi::static_buff<int, 24> spc_idx{};
                struct bufs {
                    bufs() = default;
                    dfi::static_buff<char, 16> buff{};
                    tk_t t{tk_t::none};
                    ...
                    std::int64_t num() const {
                        return dfi::str_util::atoi(std::string_view{buff.data(), buff.size()});
                    }
                };
                dfi::static_buff<bufs, 24> b{};
                    ...
                    if(w_idx.size() == 1) {
                        ...
                        if(w_idx[0] + 1 < (int)b.size() && b[w_idx[0] + 1].is_num()) {
                            if(b[w_idx[0] + 1].buff.size() == 2) {
                                ...
                                res_week = dfi::str_util::atoi(wwd.substr(0, 2));
                                res_week_day = dfi::str_util::atoi(wwd.substr(2));
                                have_yw = false;
                                have_ywd = true;
                                min_time_index = w_idx[0] + 2;
                            }
                        ...
                        if(minus_idx.size() > 0) {
                            ...
                            if(
                                ...
                                res_year = dfi::str_util::atoi(yyyymmdd.substr(0, 4));
                                res_month = dfi::str_util::atoi(yyyymmdd.substr(4, 2));
                                res_day = dfi::str_util::atoi(yyyymmdd.substr(6, 2));
                                have_ymd = true;
                                ...
                                res_year = dfi::str_util::atoi(yyyyDDD.substr(0, 4));
                                res_year_day = dfi::str_util::atoi(yyyyDDD.substr(4, 3));
                                have_yod = true;
                                if(min_time_index < 0) { min_time_index = date_index + 1; }
                            }
                        }
                ...
                if(min_time_index >= 0 && min_time_index < (int)b.size()) {
                    ...
                    if(colon_idx.size() >= 1) {
                        ...
                        if(
                            ...
                            res_hour = dfi::str_util::atoi(hhmmss.substr(0, 2));
                            res_minute = dfi::str_util::atoi(hhmmss.substr(2, 2));
                            res_second = dfi::str_util::atoi(hhmmss.substr(4, 2));
                ...
                if(have_z) {
                    ...
                    if(min_gmt_offs_start >= 0 && (int)b.size() > min_gmt_offs_start && b[min_gmt_offs_start].is_sign()) {
                        ...
                        if(
                            ...
                            res_gmt_offs_hour = dfi::str_util::atoi(val.substr(0, 2));
                            res_gmt_offs_min = dfi::str_util::atoi(val.substr(2, 2));
                            if(b[min_gmt_offs_start].is_minus()) { res_gmt_offs_sign = -1; }
                            have_gmtoffs = true;
...
inline std::ostream &operator<<(std::ostream &os, dfi::timespec_wrapper const &ts) {
