...
#include "base64.hpp"
#if defined(DFI_JSON_DEBUGGING) || defined(DFI_USE_CUSTOM_ANY)
#include "any.hpp"
#endif
...
namespace dfi {
    ...
    namespace detail {
        ...
        static bool is_ident(std::wstring const &v) {
            ...
            auto c0{v[0]};
            if(!(c0 == '_' || c0 == '$' || dfi::str_util::fltr<std::wstring>::isalpha(c0))) { return false; }
            for(auto &&c: v) { if(!(c == '_' || c == '$' || dfi::str_util::fltr<std::wstring>::isalnum(c))) { return false; } }
        ...
        public:
            ...
            void accept_token(dfi::fsm_tokenizer<std::wstring>::token const &o) {
                auto t = o.type;
                if(t != L"spc") {
                    auto k = o.value;
                    if(t == L"str" || t == L"sts") {
            ...
            void consume_token(std::wstring const &tk, std::wstring const &tt, int line, int col) {
                ...
                if(
                    ...
                    (
                        ...
                        !nxt_lst_->contains(dfi::str_util::to_utf8(tt))
#else
                        nxt_lst_->find(dfi::str_util::to_utf8(tt)) == nxt_lst_->end()
#endif
                    )
                    ||
                    ...
                    ss << ", got: \"" << dfi::str_util::to_utf8(tk) << "\" (" << exp_map.at(dfi::str_util::to_utf8(tt)) << ")";
                ...
                if(tt == L"str" || tt == L"sts") {
                    ...
                    if(top().obj.is_array()) {
                        ...
                        push();
                        top().name = dfi::str_util::to_utf8(detail::wunescape(tk));
                        ++top().obj_itm_phase;
                        what_next_ = expected::listed;
                        nxt_lst_ = &next_list_str_o_;
                    } else if(top().obj_itm_phase == 2) {
                        top().obj = dfi::str_util::to_utf8(detail::wunescape(tk));
                        ++top().obj_itm_phase;
                        what_next_ = expected::listed;
                    ...
                    std::int64_t v{dfi::str_util::atoi(tk)};
                    ...
                    std::int64_t v{dfi::str_util::atoi(tk.substr(2), 16)};
                    if(neg_accordingly_sgn_stack()) { v = -v; }
                    if(top().obj.is_array()) {
                        top().obj.push_back(v);
                        what_next_ = expected::listed;
                    ...
                    throw json_error{std::string{"json error: invalid token: \""} + dfi::str_util::to_utf8(tk) + "\""};
                }
                collapse_top_down();
            }
    ...
    public:
        ...
        json(wchar_t const *v): v_{dfi::str_util::to_utf8(v)}, t_{jo_string} {}
        json(std::wstring const &v): v_{dfi::str_util::to_utf8(v)}, t_{jo_string} {}
        json(std::wstring_view v): v_{dfi::str_util::to_utf8((std::wstring{v}))}, t_{jo_string} {}
        ...
        dfi::bytevec as_bytevec() const {
            if(is_string()) {
                return dfi::base64_str_to_data(as<std::string>());
            } else {
        ...
        bool as_boolean() const {
            if(is_bool()) {
                ...
                if(dfi::str_util::fltr<std::string>::strtolower(s) == "true") {
                    return true;
                } else if(dfi::str_util::fltr<std::string>::strtolower(s) == "false") {
                    return false;
                } else {
                    return s.size() != 0;
                }
        ...
        std::wstring as_wstring() const {
            return dfi::str_util::from_utf8(as_string());
        }
        ...
        float try_as_float() const {
            if(t_ == jo_flt) {
                ...
                return dfi::str_util::atof(as<std::string>());
            }
            throw json_error{"json error: invalid json state"};
        }
        ...
        static json deserialize(dfi::bytevec const &s) {
        ...
        static json deserialize(std::string const &s) {
            ...
            dfi::fsm_tokenizer<std::wstring> t{
                parse_rules_5,
                [&](dfi::fsm_tokenizer<std::wstring>::token const &a) { d.accept_token(a); }
            };
            for(size_t pos{0}; pos < s.size();) {
                int increment{};
                std::int64_t uc{dfi::str_util::utf8_to_ucs(s.data() + pos, &increment)};
                if(uc < 0) {
                    ++pos;
                    uc = '?';
                } else {
        ...
        static json bdeserialize(std::vector<std::uint8_t> const &vk) {
            dfi::serial_reader sr{vk.data(), vk.size()};
            auto iter{sr.begin()};
    ...
    private:
...
#endif
        void bserialize_actual(dfi::serializer &ser) const {
            if(!type_valid()) {
        ...
        static json bdeserialize_actual(dfi::serial_reader::const_iterator &iter) {
            json res{};
