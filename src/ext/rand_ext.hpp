...
namespace dfi {
    ...
    public:
        ...
        void register_runtime(runtime_interface *rt) override {
            ...
            rt->add_function("normal_dist_prng", DFIFUN(args) {
                if(args.size() == 1) {
                    return dfi::valbox{std::make_shared<norm_dist_gen<long double>>(args[0].cast_to_long_double()), "normal_dist_prng"};
                } else if(args.size() == 2) {
                    return dfi::valbox{std::make_shared<norm_dist_gen<long double>>(args[0].cast_to_long_double(), args[1].cast_to_long_double()), "normal_dist_prng"};
                }
                return dfi::valbox{std::make_shared<norm_dist_gen<long double>>(), "normal_dist_prng"};
            });
            rt->add_method("normal_dist_prng", "gen", DFIFUN(args) {
                if(args.size() == 1) {
                    return DFITHIS(args, std::shared_ptr<norm_dist_gen<long double>>)->gen();
                }
