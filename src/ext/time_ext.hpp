...
namespace dfi {
    ...
    public:
        ...
        void register_runtime(runtime_interface *rt) override {
            ...
            rt->add_function("timestamp", DFIFUN(args) {
                std::string s{};
                if(args.size() > 0) {
                    if(args[0].is_string()) {
                        return dfi::valbox{timespec_wrapper{args[0].as_string()}, "timestamp"};
                    } else if(args[0].is_wstring()) {
                        return dfi::valbox{timespec_wrapper{args[0].as_wstring()}, "timestamp"};
                    } else if(args[0].is_any_fp_number()) {
                        return dfi::valbox{timespec_wrapper{args[0].cast_to_long_double()}, "timestamp"};
                    } else if(args[0].is_any_int_number()) {
                        return dfi::valbox{timespec_wrapper{args[0].cast_to_s64()}, "timestamp"};
                    }
                }
                return dfi::valbox{timespec_wrapper::now(), "timestamp"};
            });
            ...
            rt->add_object_deserializer("timestamp",
                [](std::string const &class_name, std::string const &serial_form) -> valbox {
                    if(class_name == "timestamp") {
                        return dfi::valbox{timespec_wrapper{serial_form}, "timestamp"};
                    }
                    return dfi::valbox{};
                }
            );
            ...
            rt->add_function("gmtimestamp", DFIFUN() { return dfi::valbox{timespec_wrapper::gmtnow(), "timestamp"}; });
            rt->add_method("timestamp", "year", DFIFUN(args) { return DFITHIS(args, timespec_wrapper).year(); });
            rt->add_method("timestamp", "month", DFIFUN(args) { return DFITHIS(args, timespec_wrapper).month(); });
            rt->add_method("timestamp", "day", DFIFUN(args) { return DFITHIS(args, timespec_wrapper).day(); });
            rt->add_method("timestamp", "weekday", DFIFUN(args) { return DFITHIS(args, timespec_wrapper).weekday(); });
            rt->add_method("timestamp", "hour", DFIFUN(args) { return DFITHIS(args, timespec_wrapper).hour(); });
            rt->add_method("timestamp", "min", DFIFUN(args) { return DFITHIS(args, timespec_wrapper).minute(); });
            rt->add_method("timestamp", "sec", DFIFUN(args) { return DFITHIS(args, timespec_wrapper).sec_with_subsec(); });
            rt->add_method("timestamp", "seconds", DFIFUN(args) { return DFITHIS(args, timespec_wrapper).seconds(); });
            rt->add_method("timestamp", "fseconds", DFIFUN(args) { return DFITHIS(args, timespec_wrapper).fseconds(); });
            rt->add_method("timestamp", "to_string", DFIFUN(args) {
                std::size_t prec{static_cast<std::size_t>(9)};
                if(args.size() > 1) { prec = args[1].cast_num_to_num<std::size_t>(); }
                return DFITHIS(args, timespec_wrapper).as_iso_8601_str(prec);
            });
            rt->add_method("timestamp", "as_iso_8601", DFIFUN(args) {
                std::size_t prec{static_cast<std::size_t>(9)};
                if(args.size() > 1) { prec = args[1].cast_num_to_num<std::size_t>(); }
                return DFITHIS(args, timespec_wrapper).as_iso_8601_str(prec);
            });
            rt->add_method("timestamp", "as_gmt_iso_8601", DFIFUN(args) {
                std::size_t prec{static_cast<std::size_t>(9)};
                if(args.size() > 1) { prec = args[1].cast_num_to_num<std::size_t>(); }
                return DFITHIS(args, timespec_wrapper).as_gmt_iso_8601_str(prec);
            });
            rt->add_method("timestamp", "from_string", DFIFUN(args) {
                std::string s{}; if(args.size() > 1) { s = args[1].cast_to_string(); }
                DFITHIS(args, timespec_wrapper).from_iso_8601(s);
                return args[0];
            });
            rt->add_function("steady_clock", DFIFUN() {
                return static_cast<long double>(std::chrono::steady_clock::now().time_since_epoch().count()) * 1e-9L;
            });
            rt->add_function("highres_clock", DFIFUN() {
                return std::chrono::duration<long double, std::nano>{std::chrono::high_resolution_clock::now().time_since_epoch()}.count() * 1e-9L;
            });
            rt->add_function("localtime", DFIFUN() {
                return timespec_wrapper::now().fseconds();
            });
            rt->add_function("gmtime", DFIFUN() {
