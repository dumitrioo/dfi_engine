...
namespace dfi {
    ...
    public:
        ...
        void register_runtime(runtime_interface *rt) override {
            ...
            rt->add_method("persistence", "open", DFIFUN(args) {
                DFI_CHCK_FUN_PARMS_NUM_EQ(args, 2)
                return DFITHIS(args, std::shared_ptr<persistence>)->open(args[1].cast_to_string());
            });
            rt->add_method("persistence", "loaded", DFIFUN(args) {
                DFI_CHCK_FUN_PARMS_NUM_EQ(args, 1)
                return DFITHIS(args, std::shared_ptr<persistence>)->loaded();
            });
            rt->add_method("persistence", "get", DFIFUN(args) {
                DFI_CHCK_FUN_PARMS_NUM_EQ(args, 2)
                return DFITHIS(args, std::shared_ptr<persistence>)->get(args[1].cast_to_string());
            });
            rt->add_method("persistence", "put", DFIFUN(args) {
                DFI_CHCK_FUN_PARMS_NUM_EQ(args, 3)
                return DFITHIS(args, std::shared_ptr<persistence>)->put(args[1].cast_to_string(), args[2]);
            });
            rt->add_method("persistence", "close", DFIFUN(args) {
                DFI_CHCK_FUN_PARMS_NUM_EQ(args, 1)
                if(args.size() == 1) {
                    return DFITHIS(args, std::shared_ptr<persistence>)->close();
                }
                return false;
            });
        }
