...
namespace dfi {
    ...
    public:
        ...
        void register_runtime(runtime_interface *rt) override {
            ...
            rt->add_function("perf_stat", DFIFUN() {
                return dfi::valbox{std::make_shared<cpu_pc<double>>(), "perf_stat"};
            });
            rt->add_method("perf_stat", "start", DFIFUN(args) {
                return DFITHIS(args, std::shared_ptr<cpu_pc<double>>)->start();
            });
            rt->add_method("perf_stat", "stop", DFIFUN(args) {
                DFITHIS(args, std::shared_ptr<cpu_pc<double>>)->stop();
                return 0;
            });
            rt->add_method("perf_stat", "cpu_load", DFIFUN(args) {
                return DFITHIS(args, std::shared_ptr<cpu_pc<double>>)->cpu_load();
            });
            rt->add_method("perf_stat", "self_cpu_consumption", DFIFUN(args) {
                return DFITHIS(args, std::shared_ptr<cpu_pc<double>>)->self_cpu_consumption();
            });
        }
    ...
    private:
        ...
        private:
            bool cpu_state_init() {
                ...
                if(GetProcessTimes(hProcess, &CreationTime, &ExitTime, &KernelTime, &UserTime)) {
                    dfi::timespec_wrapper now{ dfi::timespec_wrapper::now() };
                    curr_cpu_kernel_last_scan_time_ = dfi::timespec_wrapper{KernelTime};
                    curr_cpu_user_last_scan_time_ = dfi::timespec_wrapper{UserTime};
            ...
            void actualize_curr_cpu_value() {
                ...
                if(GetProcessTimes(GetCurrentProcess(), &CreationTime, &ExitTime, &KernelTime, &UserTime)) {
                    dfi::timespec_wrapper now{ dfi::timespec_wrapper::now() };
                    dfi::timespec_wrapper TSWKernelTime{ KernelTime };
                    dfi::timespec_wrapper TSWUserTime{ UserTime };
                    dfi::timespec_wrapper dt{ now - curr_cpu_milestone_last_scan_time_ };
                    if(/*TSWKernelTime != curr_cpu_kernel_last_scan_time_ ||
                    TSWUserTime != curr_cpu_user_last_scan_time_ ||*/
                        dt.fseconds() > 1
                        ) {
                        dt *= n_of_processors_;
                        dfi::timespec_wrapper dt_k{ TSWKernelTime - curr_cpu_kernel_last_scan_time_ };
                        dfi::timespec_wrapper dt_u{ TSWUserTime - curr_cpu_user_last_scan_time_ };
        ...
        private:
            ...
            dfi::scalar_circular_buffer<FLT_T, 5> self_cpu_consumption_buff_{};
#if defined(PLATFORM_LINUX)
            struct tms tsmpl_last_{};
#elif defined(PLATFORM_WINDOWS)
            dfi::timespec_wrapper curr_cpu_milestone_last_scan_time_{};
            dfi::timespec_wrapper curr_cpu_kernel_last_scan_time_{};
            dfi::timespec_wrapper curr_cpu_user_last_scan_time_{};
#endif
