...
namespace dfi {
    ...
    public:
        ...
        void register_runtime(runtime_interface *rt) override {
            ...
            rt_->add_function("LLA", DFIFUN(args) {
                DFI_CHCK_FUN_PARMS_NUM_IN_RANGE(args, 0, 7)
                if(args.size() == 0) {
            ...
            rt_->add_method("LLA", "latitude", DFIFUN(args) {
                DFI_CHCK_FUN_PARMS_NUM_EQ(args, 1)
                return DFITHIS(args, LLA).latitude();
            });
            rt_->add_method("LLA", "longitude", DFIFUN(args) {
                DFI_CHCK_FUN_PARMS_NUM_EQ(args, 1)
                return DFITHIS(args, LLA).longitude();
            });
            rt_->add_method("LLA", "altitude", DFIFUN(args) {
                DFI_CHCK_FUN_PARMS_NUM_EQ(args, 1)
                return DFITHIS(args, LLA).altitude();
            });
            rt_->add_method("LLA", "to_string_traditional", DFIFUN(args) {
                DFI_CHCK_FUN_PARMS_NUM_IN_RANGE(args, 1, 2)
                if(args.size() == 2) {
                    return DFITHIS(args, LLA).to_string_traditional(args[1].cast_to_size_t());
                } else {
                    return DFITHIS(args, LLA).to_string_traditional();
                }
            });
            rt_->add_method("LLA", "to_string_decimal_degrees", DFIFUN(args) {
                DFI_CHCK_FUN_PARMS_NUM_EQ(args, 1)
                return DFITHIS(args, LLA).to_string_decimal_degrees();
            });
            rt_->add_method("LLA", "to_string_iso_6709", DFIFUN(args) {
                DFI_CHCK_FUN_PARMS_NUM_EQ(args, 1)
                return DFITHIS(args, LLA).to_string_iso_6709();
            });
            rt_->add_method("LLA", "distance_to", DFIFUN(args) {
                DFI_CHCK_FUN_PARMS_NUM_EQ(args, 2)
                return DFITHIS(args, LLA).distance_to(DFICLASSARG(args, 1, LLA));
            });

            rt_->add_function("ECEF", DFIFUN(args) {
            ...
            rt_->add_method("ECEF", "x", DFIFUN(args) {
                DFI_CHCK_FUN_PARMS_NUM_EQ(args, 1)
                return DFITHIS(args, ECEF).x();
            });
            rt_->add_method("ECEF", "y", DFIFUN(args) {
                DFI_CHCK_FUN_PARMS_NUM_EQ(args, 1)
                return DFITHIS(args, ECEF).y();
            });
            rt_->add_method("ECEF", "z", DFIFUN(args) {
                DFI_CHCK_FUN_PARMS_NUM_EQ(args, 1)
                return DFITHIS(args, ECEF).z();
            });

            rt_->add_function("ENU", DFIFUN(args) {
                DFI_CHCK_FUN_PARMS_NUM_IN_RANGE(args, 0, 3)
                if(args.size() == 0) {
            ...
            rt_->add_method("ENU", "east", DFIFUN(args) {
                DFI_CHCK_FUN_PARMS_NUM_EQ(args, 1)
                return DFITHIS(args, ENU).east();
            });
            rt_->add_method("ENU", "north", DFIFUN(args) {
                DFI_CHCK_FUN_PARMS_NUM_EQ(args, 1)
                return DFITHIS(args, ENU).north();
            });
            rt_->add_method("ENU", "up", DFIFUN(args) {
                DFI_CHCK_FUN_PARMS_NUM_EQ(args, 1)
                return DFITHIS(args, ENU).up();
            });

            rt_->add_function("LLAtoECEF", DFIFUN(args) {
                DFI_CHCK_FUN_PARMS_NUM_EQ(args, 1)
                return valbox{LLAtoECEF(DFICLASSARG(args, 0, LLA)), "ECEF"};
            });
            rt_->add_function("ECEFtoLLA", DFIFUN(args) {
                DFI_CHCK_FUN_PARMS_NUM_EQ(args, 1)
                return valbox{ECEFtoLLA(DFICLASSARG(args, 0, ECEF)), "LLA"};
            });
            rt_->add_function("ECEFtoENU", DFIFUN(args) {
                DFI_CHCK_FUN_PARMS_NUM_EQ(args, 2)
                return valbox{ECEFtoENU(DFICLASSARG(args, 0, ECEF), DFICLASSARG(args, 1, LLA)), "ENU"};
            });
        }
