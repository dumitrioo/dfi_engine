...
#include "../../src/dfi_value.hpp"
#include "../../src/dfi_util.hpp"

#include <zmq.h>

class zmq_ext: public dfi::extension_interface {
public:
    zmq_ext() = default;
    ...
    void register_runtime(dfi::runtime_interface *rt) override {
        ...
        rt->add_object_unary_operation("zmq_ctx", std::string{dfi::OPERATOR_CAST_TO_BOOL},
            [](dfi::valbox &v) -> dfi::valbox {
               if(v.class_name() == "zmq_ctx") {
                   return v.as_class<std::shared_ptr<void>>().get() != nullptr;
               }
               throw std::runtime_error{"invalid operand"};
            }
        );

        rt->add_object_unary_operation("zmq_sock", std::string{dfi::OPERATOR_CAST_TO_BOOL},
            [](dfi::valbox &v) -> dfi::valbox {
               if(v.class_name() == "zmq_sock") {
                   return v.as_class<std::shared_ptr<void>>().get() != nullptr;
               }
               throw std::runtime_error{"invalid operand"};
            }
        ...
        rt->add_function("zmq_errno", DFIFUN() { return zmq_errno(); });
        rt->add_function("zmq_errstr", DFIFUN() { return zmq_strerror(zmq_errno()); });
        rt->add_function("zmq_err_to_str", DFIFUN(args) { return zmq_strerror(args[0].cast_num_to_num<int>()); });
        rt->add_function("zmq_ctx_new", DFIFUN() {
            return dfi::valbox{std::shared_ptr<void>{
                zmq_ctx_new(),
                [](void *c) { zmq_ctx_destroy(c); }
            }, "zmq_ctx"};
        });
        rt->add_function("zmq_ctx_term", DFIFUN(args) {
            return zmq_ctx_term(DFITHIS(args, std::shared_ptr<void>).get());
        });
        rt->add_function("zmq_ctx_shutdown", DFIFUN(args) { return zmq_ctx_shutdown(DFITHIS(args, std::shared_ptr<void>).get()); });
        rt->add_function("zmq_ctx_set", DFIFUN(args) { return zmq_ctx_set(DFITHIS(args, std::shared_ptr<void>).get(), args[1].cast_num_to_num<int>(), args[2].cast_num_to_num<int>()); });
        rt->add_function("zmq_ctx_get", DFIFUN(args) { return zmq_ctx_get(DFITHIS(args, std::shared_ptr<void>).get(), args[1].cast_num_to_num<int>()); });
        rt->add_function("zmq_socket", DFIFUN(args) {
            return dfi::valbox{std::shared_ptr<void>{
                zmq_socket(DFITHIS(args, std::shared_ptr<void>).get(), args[1].cast_num_to_num<int>()),
                [](void *s) { zmq_close(s); }
            }, "zmq_sock"};
        });
        rt->add_function("zmq_bind", DFIFUN(args) { return zmq_bind(DFITHIS(args, std::shared_ptr<void>).get(), args[1].cast_to_string().c_str()); });
        rt->add_function("zmq_connect", DFIFUN(args) { return zmq_connect(DFITHIS(args, std::shared_ptr<void>).get(), args[1].cast_to_string().c_str()); });

        rt->add_function("zmq_setsockopt", DFIFUN(args) {
            DFI_CHCK_FUN_PARMS_NUM_IN_RANGE(args, 3, 3)
            int opt{args[1].cast_num_to_num<int>()};
            auto d{args[2].cast_to_byte_array()};
            return zmq_setsockopt(DFITHIS(args, std::shared_ptr<void>).get(), opt, d.data(), d.size());
        });

        rt->add_function("zmq_z85_encode", DFIFUN(args) {
        ...
        rt->add_function("zmq_z85_decode", DFIFUN(args) {
            DFI_CHCK_FUN_PARMS_NUM_IN_RANGE(args, 1, 1)
        ...
        rt->add_function("zmq_send", DFIFUN(args) {
            DFI_CHCK_FUN_PARMS_NUM_IN_RANGE(args, 2, 3)
            std::vector<std::uint8_t> buf{args[1].cast_to_byte_array()};
            if(args.size() == 3) {
                return zmq_send(DFITHIS(args, std::shared_ptr<void>).get(), buf.data(), buf.size(), args[2].cast_num_to_num<int>());
            } else {
                return zmq_send(DFITHIS(args, std::shared_ptr<void>).get(), buf.data(), buf.size(), 0);
            }
        });
        rt->add_function("zmq_send_more", DFIFUN(args) {
            DFI_CHCK_FUN_PARMS_NUM_IN_RANGE(args, 2, 2)
            std::vector<std::uint8_t> buf{args[1].cast_to_byte_array()};
            return zmq_send(DFITHIS(args, std::shared_ptr<void>).get(), buf.data(), buf.size(), ZMQ_MORE);
        });
        rt->add_function("zmq_recv", DFIFUN(args) {
            ...
            if(args.size() > 1) {
                ...
                buf.resize(nread);
                if(args.size() == 3) {
                    nrd = zmq_recv(DFITHIS(args, std::shared_ptr<void>).get(), buf.data(), nread, args[2].cast_num_to_num<int>());
                } else {
                    nrd = zmq_recv(DFITHIS(args, std::shared_ptr<void>).get(), buf.data(), nread, 0);
                }
            } else {
                size_t nread{32 * 1024};
                buf.resize(nread);
                nrd = zmq_recv(DFITHIS(args, std::shared_ptr<void>).get(), buf.data(), nread, 0);
            }
